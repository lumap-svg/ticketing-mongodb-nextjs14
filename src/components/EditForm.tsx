...
export default async function EditForm({ id }: { id: string }) {
  ...
  const ticket: ticketT = await getTicket(id);
  const { title, description, status, progress, priority } = ticket;
  console.log(title, description, status, progress, priority);
  return (
    <>
      {
        ticket && (
          <div>
            <h2>{title}</h2>
            <p>{description}</p>
            {/* title, description,status, progress, priority */}
          </div>
        )
        //   (
        //     <form
